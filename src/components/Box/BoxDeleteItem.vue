<template>
    <b-container id="item-change">
            ...
            <b-row class="p-1">
                ...
                <b-col sm="10">
                    ...
                    <b-form-invalid-feedback :state="valid">
                        Вещь в коробке отсутствует или Неправильный ID номер
                    </b-form-invalid-feedback>
                </b-col>
            </b-row>
            <b-row align-h="center" class="p-1">
                <b-button type="submit" @click="searchItem" block variant="secondary" size="lg">Найти</b-button>
            </b-row>
            <b-row align-h="center" class="p-1">
                <b-button @click="$router.push('/box/edit/' + $route.params.id)" block variant="danger" size="lg">
                    Назад
                </b-button>
            </b-row>
        </template>
        <template v-else>
            ...
            <b-row align-h="center" class="p-1">
                <b-button type="submit" @click="deleteItemFromBox" block variant="secondary" size="lg">Удалить
                </b-button>
            </b-row>
            <b-row align-h="center" class="p-1">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            deleteItemFromBox() {
                let self = this;
                axios.delete(config.vueConfig.ip + '/box/' + self.$route.params.id + '/item/' + self.itemId).then(() => {
                    self.$router.push('/box/edit/' + self.$route.params.id)
                })
            },
            searchItem() {
                let self = this;
                if (this.itemId) {
                    if (this.itemId) {
                        axios.get(config.vueConfig.ip + '/item/' + this.itemId + '/box/' + self.$route.params.id).then((response) => {
                            if (response.data.id) {
